...
object ObpAPI extends Loggable {
  ...
  //These collections can only be generated by props, so we set it to the val can be used all the time.
  val sharableApiCollections = {
    val apiCollectionId = Helper.getPropsValue("api_collection_ids","").split(",").map(_.trim).toList
    apiCollectionId.map(apiCollectionId => ObpGet(s"$obpPrefix/v4.0.0/api-collections/sharable/$apiCollectionId").flatMap(_.extractOpt[ApiCollectionJson400])).filter(_.isDefined).map(_.head)
      .map(apiCollection => (apiCollection.api_collection_name, apiCollection.api_collection_id))
  } 
